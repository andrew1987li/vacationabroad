...
<%@ Page Language="C#" MasterPageFile="~/MasterPageNoCss7.master" AutoEventWireup="true"
    CodeFile="RegionList7.aspx.cs" Inherits="RegionList7" Title="<%# GetTitle () %>" %>

<%@ OutputCache Duration="600" VaryByParam="*" %>
<asp:Content ID="Content" ContentPlaceHolderID="Content" runat="Server">
    <div style="height: 413px;">
    </div>
    <asp:Repeater ID="Repeater1" runat="server" DataMember="Countries" DataSource="<%# CountriesStates %>"
        Visible="false">
        <HeaderTemplate>
            <table width="97%" cellpadding="0" cellspacing="2" border="0" bgcolor="#ffffff" align="center">
                <tr>
                    <td colspan="25">
                        <h1>
                            <p align="center">
                ...
                <td valign="top">
                    <div style="width: 130px; text-align: left;">
                        <font size="2"><a href='<%# CommonFunctions.PrepareURL (((string)DataBinder.Eval(Container.DataItem, "Country", "{0}")).Replace (" ", "_").ToLower () + "/default.aspx") %>'>
                            <b>
                                <%# DataBinder.Eval(Container.DataItem, "Country", "{0}") %></b> </a></font>
                    </div>
                </td>
                <td>
                        ...
                        <asp:Repeater ID="Repeater2" runat="server" DataSource='<%# ((System.Data.DataRowView)Container.DataItem).CreateChildView("CountriesStates")%>'>
                            ...
                            <ItemTemplate>
                                <font size="2"><a href='<%# CommonFunctions.PrepareURL (((string)DataBinder.Eval(Container.DataItem, "Country", "{0}")).Replace (" ", "_").ToLower () + "/" + ((string)DataBinder.Eval(Container.DataItem, "StateProvince", "{0}")).Replace (" ", "_").ToLower () + "/default.aspx") %>'>
                                    <b>
                                        <%# DataBinder.Eval(Container.DataItem, "StateProvince", "{0}") %></a></font>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            if ($("input[id$='kuchb']").val() != '') {
                var regionCode = $("input[id$='kuchb']").val();
    ...
    <noscript>
        <div class="statcounter">
            <a href="http://www.statcounter.com/" target="_blank">
                <img class="statcounter" src="http://c.statcounter.com/3812793/0/e3bbd0d0/1/" alt="web statistic"></a></div>
    </noscript>
    <!-- End of StatCounter Code -->
    <asp:HiddenField runat="server" ID="kuchb" />
</asp:Content>
